...
// When React.memo() wraps a component, React memoizes the rendered output of the wrapped component
// then skips unnecessary renderings.
const Tweets = memo(({ tweetService, username, addable }) =>  {
  ...
  return (
    <>
      ...
      {tweets.length === 0 && <p className='tweets-empty'>No Tweets Yet</p>} // tweets = state =[]
      <ul className='tweets'>
        {tweets.map((tweet) => (
          <TweetCard
            key={tweet.id}
